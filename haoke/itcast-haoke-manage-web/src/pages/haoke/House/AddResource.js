...
class AddResource extends PureComponent {
    handleSubmit = e => {
        ...
        e.preventDefault();//拦截submit提交 目的是在提交表单前做一些事
        //通过form.validateFieldsAndScroll()对表单进行校验。
        //通过values获取表单中输入的值。
        //通过dispatch()调用model中定义的方法。
        form.validateFieldsAndScroll((err, values) => {
            if (!err) {
              if(values.facilities){
                values.facilities = values.facilities.join(",");//配套设施，如：1,2,3
              }
              if(values.floor_1 && values.floor_2){
                values.floor = values.floor_1 + "/" + values.floor_2;//楼层，如：8/26
              ...
              // 处理图片(图片存储到数据库中，是使用逗号分割的)
              values.pic = [...this.state.pics].join(',');//把Set集合转成数组(用扩展运算符把Set集合展开) 再用数组的join方法

              dispatch({
                    type: 'house/submitHouseForm',//表示调用命名空间为house的model下的submitHouseForm方法
                    payload: values,
                });
            }
        });
    ...
    handleSearch = (value)=>{//value拿到的就是我们输入的值
      let arr = new Array();
      if(value.length > 0 ){
  ...
  //这个方法中 遍历了图片列表 并把name属性加入到Set集合中 实现去重 最后把集合丢到state中
  handleFileList = (obj)=>{
    let pics = new Set();
    obj.forEach((v, k) => {
       if(v.response){
  ...
  constructor(props){
    ...
    render() {
        ...
        return (
            <PageHeaderWrapper>
                //在提交表单的时候 调用handleSubmit方法 handleSubmit方法位于class内部 可以使用this.调用
                <Form onSubmit={this.handleSubmit} hideRequiredMark style={{ marginTop: 8 }}>
                    <Card bordered={false} title="房源信息">
                        ...
                        <FormItem {...formItemLayout} label="房源标题">
                          //input和title值做双向绑定。 通过设置和获取title值，可以获取和修改这个input的值。 在getFieldDecorator的参数中，还添加了校验规则。
