...
public class OrderServiceImpl implements OrderService {
    ...
    public void closeOrder(String orderId) {
         ...
         * 2. 基于微信查询订单信息(微信)
         *  1)如果当前订单的支付状态为已支付,则进行数据补偿(mysql) 即记录订单日志 修改支付状态为已支付
         *  2)如果当前订单的支付状态为未支付,则修改mysql中的订单信息,新增订单日志 恢复商品的库存,基于微信关闭订单
         */
        ...
        /*
        if (!"0".equals(order.getPayStatus())){ //0未支付 1已支付
       ...
       //如果订单的支付状态为已支付,进行数据补偿(mysql)
        ...
        if ("NOTPAY".equals(wxQueryMap.get("trade_state"))){
            ...
            //恢复商品的库存 (根据订单ID 查出订单项列表 订单项=商品SKU)
    ...
    public void batchSend(List<Order> orders) {
        ...
        /**
         * 写了三个for循环的原因（使用单个for循环的问题）:
         * 比如集合中有100个订单 如果只有一个循环操作的话
         * 前面99个订单可能都是符合条件的并且修改成功
         * 但是最后一个订单出现了问题（校验不通过或出现了异常）
         * 就会导致前面99个订单信息都会出现回滚
         *
         * 使用三个for循环 如果有一个出现问题 就不用再做后续的操作了
         * 只有全部通过 才会进入到下一个for循环中
         */
    }
    ...
    public void confirmTask(String orderId, String operator) {
        ...
        if (!"1".equals(order.getConsignStatus())){ //1 表示已发货
    ...
    public void autoTack() {
        ...
        //LocalDate得到的是日期  LocalDateTime得到的是日期+时间
        //获取当前时间
        ...
        criteria.andLessThan("consignTime",date); //获取在这个日期之前的订单
        criteria.andEqualTo("orderStatus","2");
        List<Order> orderList = orderMapper.selectByExample(example);

        for (Order order : orderList) {
            this.confirmTask(order.getId(),"system"); //调用 确认收货
        }
